...
func (c *client) mqttParse(buf []byte) error {
	...
	for err == nil && r.hasMore() {
		...
		case mqttPacketConnect:
			...
			if err == nil {
				rc, sessp, err = s.mqttProcessConnect(c, cp)
				if err == nil {
					connected = true
					rd = cp.rd
				}
...
func (as *mqttAccountSessionManager) processSubs(sess *mqttSession, clientID string, c *client,
	filters []*mqttFilter, updateSession bool) ([]*subscription, error) {
	...
	for _, f := range filters {
		...
		if err == nil {
			...
			jscons, jssub, err = c.mqttProcessJSConsumer(sess, as.mstream, subject, sid, f.qos)
		}
		if err != nil {
		...
		if mqttNeedSubForLevelUp(subject) {
			...
			if err == nil {
				fwjscons, fwjssub, err = c.mqttProcessJSConsumer(sess, as.mstream, fwcsubject, fwcsid, f.qos)
			}
			if err != nil {
...
func (s *Server) mqttProcessConnect(c *client, cp *mqttConnectProto) (byte, bool, error) {
	c.mu.Lock()
	c.clearAuthTimer()
	c.mu.Unlock()
	if !s.isClientAuthorized(c) {
		return mqttConnAckRCNotAuthorized, false, ErrAuthentication
	}
	...
	return mqttConnAckRCConnectionAccepted, sessp, nil
}
...
//
// Account session manager lock held on entry.
func (c *client) mqttProcessJSConsumer(sess *mqttSession, stream *Stream, subject, sid string, qos byte) (*Consumer, *subscription, error) {
	...
	if exists && qos == 0 {
		...
		// the form: "$MQTT_sub.<nuid>". It is also used as the sid
